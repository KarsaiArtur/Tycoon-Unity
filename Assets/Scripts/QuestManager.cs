...
public class QuestManager : MonoBehaviour, Saveable
{
    ...
    void Start()
    {
        ...
        easyQuests.Add(new Quest("Introduce Your First Residents", "Have 15 animals of any species", 1000, "Easy",
            () => { var animalCount = 0; foreach (var exhibit in GridManager.instance.exhibits) { animalCount += exhibit.GetAnimals().Count; } return animalCount >= 15; },
            () => { var animalCount = 0; foreach (var exhibit in GridManager.instance.exhibits) { animalCount += exhibit.GetAnimals().Count; } return animalCount; }, 15));
        ...
        mediumQuests.Add(new Quest("Add More Fascinating Creatures", "Have 50 animals of any species", 5000, "Medium",
            () => { var animalCount = 0; foreach (var exhibit in GridManager.instance.exhibits) { animalCount += exhibit.GetAnimals().Count; } return animalCount >= 50; },
            () => { var animalCount = 0; foreach (var exhibit in GridManager.instance.exhibits) { animalCount += exhibit.GetAnimals().Count; } return animalCount; }, 50));
        ...
        hardQuests.Add(new Quest("Wild Kingdom", "Have 100 animals of any species", 10000, "Hard",
            () => { var animalCount = 0; foreach (var exhibit in GridManager.instance.exhibits) { animalCount += exhibit.GetAnimals().Count; } return animalCount >= 100; },
            () => { var animalCount = 0; foreach (var exhibit in GridManager.instance.exhibits) { animalCount += exhibit.GetAnimals().Count; } return animalCount; }, 100));
...
///******************************
    ...
    public class QuestManagerData
    {
        ...
        public QuestManagerData(int currentQuestId, int numberOfDoneQuests, List<int> questPoolIds, bool questsCompleted)
        {
           ...
           this.questPoolIds = questPoolIds;
    ...
    void SetData(int currentQuestId, int numberOfDoneQuests, List<int> questPoolIds, bool questsCompleted){ 
